...
class Buffer:
    ...
    def worldmodel_get_prediction(self, state, action, max_retries=5):
        ...
        try:
            ...
            # world_model_query += "\nMake your prediction for 'success' and 'state 1'. You should only give the 'success' and 'state 1', with out any other information."
            ...
            # wm_prediction_dict = self.llm(messages).content
            # log_info(f"WorldModel prediction Result: {wm_prediction_dict}")
    ...
    #         return self.state_information_transfer(
    #             state,
    #             max_retries=max_retries - 1
    #         )
                    ...
                    if line.strip().startswith('>'):
                        ...
                        for action_keyword in valid_actions:
                            if stripped_line.startswith(action_keyword):
                                ...
                                ##################
                                positive=action_result
                                if positive == False: # state info中添加一个action success项，记录上一个action是否成功
                                    transition_info = {'inital_state': state, 'action': action, 'action_result': action_result}
                                    record_wrong.setdefault(action_keyword, []).append(transition_info)
                                    name_neg = action_keyword+'_negative'
                                    if name_neg in record:
